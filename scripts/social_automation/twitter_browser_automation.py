...
class TwitterBrowserBot:
    ...
    def setup_engagement_memory(self):
        """Setup memory system to track engaged posts"""
        self.memory_file = "twitter_engagement_memory.json"
        
        # Load existing memory or create new
        if os.path.exists(self.memory_file):
            with open(self.memory_file, 'r') as f:
                self.engaged_posts = json.load(f)
            print(f"📚 Loaded memory: {len(self.engaged_posts)} posts already engaged")
        else:
            self.engaged_posts = {}
            print("📚 Created new engagement memory")
    
    def save_engagement(self, post_url, post_text):
        """Save engaged post to memory"""
        self.engaged_posts[post_url] = {
            'engaged_at': datetime.now().isoformat(),
            'post_preview': post_text[:100]
        }
        
        with open(self.memory_file, 'w') as f:
            json.dump(self.engaged_posts, f, indent=2)
        
        print(f"💾 Saved to memory: {len(self.engaged_posts)} total posts")
    
    def already_engaged(self, post_url):
        """Check if we've already engaged with this post"""
        return post_url in self.engaged_posts
    ...
    async def login_to_twitter(self):
        ...
        try:
            ...
            if 'verification' in page_content.lower() or 'captcha' in page_content.lower():
                print("\n" + "="*50)
                print("🛡️ VERIFICATION DETECTED")
                print("="*50)
                print("1. Complete the verification in the browser")
                print("2. Click on this terminal window")
            ...
            confirmation = input("Are you on the Twitter homepage? (type 'yes' to continue): ").strip().lower()
            
            if confirmation == 'yes':
                print("✅ Manual login confirmed! Starting automation...")
                return True
            else:
                        ...
                        for i, tweet_element in enumerate(tweet_elements[:5]):
                            try:
                                ...
                                if text_element:
                                    ...
                                    # Get tweet URL (try to find link to tweet)
                                    tweet_url = None
                                    try:
                                    ...
                                    # Use tweet text as fallback identifier
                                    if not tweet_url:
    ...
    async def run_twitter_engagement_cycle(self):
        ...
        try:
            ...
            for i, query in enumerate(self.search_queries[:3]):  # Limit to 3 queries per cycle
                ...
                if tweets:
                    # Reply to first relevant tweet (that we haven't engaged with)
